...
class ImplicitLayer(nn.Module):
    def __init__(self, input_features, width, output_features = 1, act = torch.tanh, sigma_w = 0.6, sigma_u = 1.0, tol = None, max_iter=50, outlayer=True):
        ...
        self.preinput = nn.Linear(input_features,width,bias=False)
        init.normal_(self.preinput.weight, mean=0, std=1./sqrt(input_features)) # initialization of U
        # first layber
...
def main(args):
    ...
    df.to_csv('tmp/sim/results_sigmau.csv', mode='a', header=False)
